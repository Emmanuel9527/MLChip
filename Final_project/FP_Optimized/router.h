...
SC_MODULE( Router ) {
    ...
    */
    ...
    // Six-port router: NORTH, SOUTH, EAST, WEST, LOCAL, and HOST.
    // HOST is used by router 0 to connect the external Controller without
    // taking the LOCAL port away from PE0.
    sc_in  < bool >  reset_n;
    sc_in  < bool >  clk;
    ...
    */
    ...
    // HOST on node 0 injects to LOCAL, EAST, and SOUTH. The top row forwards
    // EAST and SOUTH; each column then forwards SOUTH. Every PE receives
    // exactly one copy.
    void build_broadcast_mask(int input_port)
    {
        ...
        if (router_id != 0 || input_port == HOST)
            broadcast_mask[input_port][LOCAL] = true;

        if (input_port == LOCAL || input_port == HOST)
        {
            if (x < 3)
    ...
    void input_thread_5() { input_port_stage(5); }
    ...
    void output_thread_5() { output_stage(5); }
